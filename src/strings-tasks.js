...
function isPalindrome(str) {
  ...
  const lastPos = chars.length - 1;
  const midPos = chars.length / 2;

  for (let i = 0; i < midPos; i += 1) {
    if (chars[i] !== chars[lastPos - i]) {
      return false;
    }
  }
...
function findLongestWord(sentence) {
  ...
  return sentence.split(' ').reduce((prev, curr) => getLongest(curr, prev), '');
}
...
function extractNameFromTemplate(value) {
  ...
  const lastName = values[2].slice(0, -1);
...
function getCardId(value) {
  const cardRanks = 'A234567891JQK';
  const cardSuits = '♣♦♥♠';
  const rank = value.slice(0, 1);
  const suit = value.slice(-1);

  return cardSuits.indexOf(suit) * cardRanks.length + cardRanks.indexOf(rank);
}
